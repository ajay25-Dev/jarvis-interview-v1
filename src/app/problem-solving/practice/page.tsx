...
export default function ProblemSolvingPracticePage() {
  ...
  const [conversationHistory, setConversationHistory] = useState<
    Array<{ role: 'student' | 'mentor'; content: string; created_at?: string }>
  >([]);
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        ...
        const data = text ? (JSON.parse(text) as { messages?: any[] }) : { messages: [] };
        const mapped =
          data.messages?.map((m) => ({
        ...
        setConversationHistory(mapped.filter((m) => m.content));
      } catch (err) {
        setMentorError(
  ...
  const handleMentorMessage = async (message: string) => {
    ...
    if (!response.ok) {
      setMentorError(text || 'Mentor chat request failed.');
